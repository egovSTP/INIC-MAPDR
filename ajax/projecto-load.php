...
$mysql  = new mysql();
$mysql->connect();
$result = $mysql->query("SELECT * FROM tentity e  WHERE e.type='projecto';");
...
if ($num > 0) {
  while ($row = mysqli_fetch_array($result)) {
    ...
    $mysql_p  = new mysql();
    $mysql_p->connect();
    $result_p = $mysql_p->query("CALL p_view_projec_content('$id')");
    while ($row_p = mysqli_fetch_array($result_p)) {
      ...
      if ($type == "Foto") {
        ...
        <a href="ler-projeto.php?p=' . $id . '&nome=' . $nome . '&sigla=' . $sigla . '">
      ...
      echo '
          ...
          <div class="info-p-title">
            <p>
              <a href="ler-projeto.php?p=' . $id . '&nome=' . $nome . '&sigla=' . $sigla . '">' . $sigla . '</a>
            </p>
            <p>
              <a href="ler-projeto.php?p=' . $id . '&nome=' . $nome . '&sigla=' . $sigla . '">' . $nome . '</a>
            </p>
          </div>
        </div>
      </div>
