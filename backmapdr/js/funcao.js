...
$(function () {
  ...
  function checkLength(o, n, min, max, tips) {
    if (o.val().length > max || o.val().length < min) {
      ...
      tips.html(
        "A longitude da " + n + " deve estar entre  " + min + " e " + max + "."
      );
      return false;
    } else {
      return true;
    ...
    buttons: {
      Autenticar: function () {
        ...
        if (usernameToLogin.val() == "") {
          ...
          bValid =
            bValid &&
            checkLength(usernameToLogin, "nome do utilizador", 3, 20, tips);
          bValid =
            bValid &&
            checkRegexp(
              usernameToLogin,
          ...
          bValid =
            bValid &&
            checkLength(passwordToLogin, "palavra passe", 6, 50, tips);
          bValid =
            bValid &&
            checkRegexp(
              passwordToLogin,
  ...
  $("#usernameToLogin").keyup(function (e) {
    ...
    if (code == 13) {
      ...
      if (usernameToLogin.val() == "") {
        ...
        bValid =
          bValid && checkLength(passwordToLogin, "palavra passe", 6, 50, tips);
        bValid =
          bValid &&
          checkRegexp(
            passwordToLogin,
  ...
  $.post(
    ...
    function (data) {
      ...
      if (result == "1") {
        ...
        $("#autenticarState").html(
          "<p>Sua conta encontra-se bloqueada, por favor contactar o Administrador!</p>"
        );
      } else {
        $("#autenticarState").addClass("ui-state-error");
...
function contains(extensions, ext) {
  ...
  for (var i = 0; i < length; i++) {
    if (extensions[i].toLowerCase() == ext.toLowerCase()) return true;
  }
  return false;
}
...
function checkImageDimension(pathName) {
  ...
  fr.onload = function () {
    ...
    img.onload = function () {
      if (img.width < img.height) {
        alert("Por favor, selecione uma image horizontal!");
      } else if (img.width >= 1024 && img.height >= 640) {
        if (pathName === "pathToInsert") {
          adicionarAsync();
        } else {
          modificarAsync();
        }
      } else {
        alert("A imagem deve ser 1024x640 minimo!");
      }
    };
    img.src = fr.result;
  };

  fr.readAsDataURL($(`#${pathName}`)[0].files[0]);
}
function checkIconDimension(pathName) {
  var fr = new FileReader();
  fr.onload = function () {
    // file is loaded
    var img = new Image();
    img.onload = function () {
      if (img.width >= 60 && img.height >= 60) {
        if (pathName === "pathToInsert") {
          adicionarCategoriaDocumentoAsync();
        } else {
          modificarCategoriaDocumentoAsync();
        }
      } else {
        alert("O icon deve ser 60x60 minimo!");
      }
    };
    img.src = fr.result;
  };
